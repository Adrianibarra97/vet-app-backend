...
class VetControllerIntegrationTest {
    ...
    @Test
    @DisplayName("Debe crear un veterinario correctamente")
    fun crear_un_veterinario_exitoso(){

        //Arrange

        val authCredentials = authCredentialsRepository.save(
            AuthCredentials(
                username = "unico_test_${UUID.randomUUID()}",
                password = "eche1234",
                typeOfUser = TypeOfUser.VET
            )
        )

        val locationInfo = locationInfoRepository.save(
            LocationInfo(
        ...
        val vetDTO = VetDTO(
            ...
            name = "Ariel",
            surname = "Tosi",
            photo = "/src/assets/nuevo.jfif",
            email = "tosi@gmail.com",
            telephone = "1147399999",
            licence = "987654321",
            speciality = "Dermatologia",
            businessHours = "9 a 18 hs",
            professionalEmail = "tosi@vetapp@gmail.com",
            professionalAddress = "Av. Siempre Viva 123",
            professionalTelephone = "1155555555",
            professionalLocality = "Buenos Aires",
            professionalPostalCode = "1000",
            idInfoLocation = locationInfo.id,
            address = locationInfo.address,
            country = locationInfo.country,
            province = locationInfo.province,
            locality = locationInfo.locality,
            postalCode = locationInfo.postalCode,
            idAuthCredentials = authCredentials.id,
            username = "unico_test_${UUID.randomUUID()}",
            password = "eche1234"
        )

        val requestJson = objectMapper.writeValueAsString(vetDTO)
        ...
        println("Respuesta del servidor: ${result.response.contentAsString}")
